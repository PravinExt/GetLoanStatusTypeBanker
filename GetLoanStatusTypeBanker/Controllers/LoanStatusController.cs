...
namespace GetLoanStatusTypeBanker.Controllers
{
    [Route("GetLoanStatusTypeBanker/[controller]")]
    [ApiController]
    public class LoanStatusController : ControllerBase
    {
        // GET: GetLoanStatusTypeBanker/LoanStatus
        [HttpGet]
        public List<LoanStatus> Get()
        {
            ...
            foreach (var itemType in Enum.GetValues(typeof(BankerLoanStaus)))
            {
                //For each value of this enumeration, add a new EnumValue instance
                values.Add(new LoanStatus()
                {
                    Status_ID = Enum.GetName(typeof(BankerLoanStaus), itemType),
                    Status = (int)itemType
                });
            }
